...
- **Predictions** (`/predictions`) — Prediction market-style page with 115 predictions across 10 categories (Economy & Finance, Technology & AI, Energy & Climate, Culture & Society, Business & Startups, Geopolitics & Governance, Education & Workforce, Infrastructure & Cities, Sports & Entertainment, Health & Demographics). Features: search bar, category filter chips with counts, stats bar, featured prediction with confidence chart, paginated grid with "Load More", and closed predictions section. Search/filter hides featured/closed sections to focus on results. Data in `src/data/predictions-data.ts`.
