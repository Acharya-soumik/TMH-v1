...
import { Linkedin, CheckCircle2, Download, Loader2 } from "lucide-react"
import { useToast } from "@/hooks/use-toast"
import { generateShareCard, generateStoryCard, shareWithImage, getXShareText } from "@/lib/shareCard"
...
interface ShareModalProps {
  ...
  body?: string
  onClose: () => void
  // Optional poll data for image generation
  category?: string
  totalVotes?: number
...
export function ShareModal({
  url, title, heading, body, onClose,
  category, totalVotes = 0, votedOptionText, votedPct, options,
}: ShareModalProps) {
  ...
  async function generateStory(): Promise<Blob | null> {
  ...
  const buildShareText = (platform: "whatsapp" | "linkedin" | "x" | "generic"): string => {
    const votePart = votedOptionText ? `I voted "${votedOptionText}"` : "Where do you stand?"
    switch (platform) {
      case "whatsapp":
        return `🔴 "${title}"\n\n${votePart} on The Tribunal — MENA's most honest opinion platform.\n\n${url}`
      case "linkedin":
        return `${votePart} on this debate from The Tribunal.\n\n"${title}"\n\n${totalVotes.toLocaleString()} voices from across MENA. Add yours: ${url}`
      case "x":
        return `${getXShareText(title, votedOptionText)}\n${url}`
      default:
        return `"${title}" — ${votePart} on The Tribunal: ${url}`
    }
  }

  const handleWhatsApp = async () => {
    const msg = buildShareText("whatsapp")
    if (hasImageData) {
  ...
  const handleLinkedIn = async () => {
    const msg = buildShareText("linkedin")
    ...
    if (hasImageData) {
      ...
      toast({ title: "Text copied!", description: "Opening LinkedIn — paste the message." })
    }
  ...
  const handleX = async () => {
    const shareText = buildShareText("x")
  ...
  const handleInstagram = async () => {
    ...
    if (hasImageData) {
      ...
      if (copyUrl) toast({ title: "Link copied!", description: "Opening Instagram — paste it in your Story or DM." })
    }
  ...
  const handleDownloadCard = async () => {
    ...
    try {
      ...
      if (blob) {
        ...
        document.body.removeChild(a)
        URL.revokeObjectURL(dlUrl)
        toast({ title: "Card saved!", description: "Share it anywhere." })
      }
    } catch {}
    setGenerating(null)
  }
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            { label: "LinkedIn", key: "linkedin", onClick: handleLinkedIn, icon: <Linkedin className="w-4 h-4 text-[#0A66C2]" /> },
            {
              label: "X", key: "x", onClick: handleX,
            ...
            {
              label: "Instagram", key: "instagram", onClick: handleInstagram,
            ...
            >
              {generating === btn.key ? <Loader2 className="w-4 h-4 animate-spin" /> : btn.icon}
        ...
        {hasImageData && (
          ...
          >
            {generating === "card" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />}
            Download Share Card
          </button>
        )}
