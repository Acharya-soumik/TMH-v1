...
export function ShareModal({ url, title, heading, body, onClose }: ShareModalProps) {
  ...
  const handleTelegram = () => {
    const msg = encodeURIComponent(`"${title}" — where does the region stand? Vote: ${url}`)
    window.open(`https://t.me/share/url?url=${encodeURIComponent(url)}&text=${msg}`, "_blank", "noopener,noreferrer")
    onClose()
  }
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-lg rounded-[14px] p-8 sm:p-10 space-y-5"
      ...
      >
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            {
              label: "Telegram", onClick: handleTelegram,
