...
export function ShareModal({ url, title, heading, body, onClose }: ShareModalProps) {
  ...
  const handleWhatsApp = async () => {
    ...
    toast({ title: "Link copied!", description: "Opening WhatsApp — paste if needed." })
  ...
  const handleLinkedIn = async () => {
    ...
    toast({ title: "Link copied!", description: "Opening LinkedIn — paste the link in your post." })
    window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}`, "_blank", "noopener,noreferrer")
    onClose()
  }

  const handleX = async () => {
    const shareText = `"${title}" — where do you stand? Vote on The Tribunal: ${url}`
    await copyToClipboard(shareText)
    toast({ title: "Link copied!", description: "Opening X — paste if needed." })
    const text = encodeURIComponent(`"${title}" — where do you stand? Vote on The Tribunal:`)
  ...
  const handleInstagram = async () => {
    ...
    if (ok) toast({ title: "Link copied!", description: "Opening Instagram — paste it in your Story or DM." })
    ...
    >
      ...
      >
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 px-5 py-4 font-black uppercase tracking-[0.15em] text-[11px] transition-colors duration-150 rounded-sm bg-[#25D366]/10 border border-[#25D366]/30 text-[#25D366] cursor-pointer hover:bg-[#25D366]/20"
        >
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            { label: "LinkedIn", onClick: handleLinkedIn, icon: <Linkedin className="w-4 h-4 text-[#0A66C2]" /> },
            {
              label: "X", onClick: handleX,
            ...
            {
              label: "Instagram", onClick: handleInstagram,
        ...
        {/* Email section — only if not already submitted */}
        {!hasEmail && (
          <>
